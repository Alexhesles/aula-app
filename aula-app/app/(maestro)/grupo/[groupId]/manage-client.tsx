...
export function ManageClient({
  ...
  function addStudents() {
    ...
    startTransition(async () => {
      ...
      const { error } = await supabase.rpc("add_students", {
        p_group_id: groupId,
        p_names: parsed,
      });
      if (error) {
      ...
      {adding ? (
        <Card className="mb-4">
          ...
          <p className="mt-1 text-sm text-ink-soft">
            Escribe o pega un nombre por línea.
          </p>
          <textarea
            value={names}
          ...
          <div className="mt-3 flex items-center gap-2">
            <Button variant="ghost" onClick={() => { setAdding(false); setNames(""); }}>
              Cancelar
            </Button>
            <Button
              fullWidth
        ...
        <Button fullWidth className="mb-4" onClick={() => setAdding(true)}>
          + Agregar alumnos
        </Button>
      )}
